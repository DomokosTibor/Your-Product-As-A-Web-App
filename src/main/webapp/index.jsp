<%@ taglib uri = "http://java.sun.com/jsp/jstl/core" prefix = "c" %>
<head>
	<title>Buy It Today!</title>
	<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-2">
	<meta name="description" content="This is a simple website where you can buy my marvelous stuff!">
	<meta name="keywords" content="domokos, tibor, marvelous, stuff, items">
	<meta name="country" content="Hungary">
	<meta name="language" content="English">
	<meta name="content-language" content="English">
	<meta http-equiv="content-language" content="en">
	<meta name="charset" content="ISO-8859-2">
	<meta name="distribution" content="global">
	<meta name="rating" content="general">
	<meta name="robots" content="index, follow">
	<meta name="robots" content="all">
	<meta name="revisit-after" content="1 Day">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<meta name="theme-color" content="#0000FF">

	<link rel="shortcut icon" type="image/icon" href="images/favicon.ico">
	<link rel="image_src" href="images/share_me.jpg">

	<link rel="stylesheet" type="text/css" href="index.css">
	<!--
	<script type="text/javascript" src="index.js"></script>
	-->
</head>

<html>
	<body>
		<header>
			<div class="name">Buy It Today!</div>
			<div class="menu">
				<ul>
					<li><a href="#">Home</a></li>
					<li><a href="#">Our products</a></li>
					<li><a href="#">Contact</a></li>
					<li><a href="#">About Us</a></li>
				</ul>
			</div>
		</header>
		<div class="main">
			<h1>Our products</h1>
			<p>Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla.</p>
			<p>Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla.</p>
			<p>Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla.</p>
			<center>
				<form action="email" method="post">
					<input type="email" name="email-address" placeholder="your@email.com" value="">
					<input type="submit" value="Submit">
				</form>
				<span style="color: red; font-size: 0.85em;">${submitSign} ${submitEmail}</span>
			</center>
			<p>
				<div class="item">
					<a href="#">
						<img src="images/item_01.jpg">
						<div class="price">$500</div>
					</a>
				</div>
				<div class="item">
					<a href="#">
					<img src="images/item_02.jpg">
					<div class="price">$600</div>
					</a>
				</div>
				<div class="item">
					<a href="#">
					<img src="images/item_03.jpg">
					<div class="price">$300</div>
					</a>
				</div>
				<div style="clear: both"></div>
			</p>
			<p>Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla.</p>
			<p>Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla. Bla bla bla.</p>
			<p style="margin-bottom: 0px; text-align: center">
				<a href="http://www.farcryhc.hu" target="_blank"><img src="http://www.farcryhc.hu/---tartalom---/banner/farcryhc_468x60_v2.png"></a>
			</p>
		</div>
		<footer>&copy; 2018 B.I.T.</footer>
	</body>
</html>
